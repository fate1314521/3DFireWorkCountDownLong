...
S.UI = (function () {
  ...
  function performAction(value) {
    ...
    // 当序列开始执行时（index=1），触发主心分散效果
    timedAction(function (index) {
      if (index === 1 && sequence.length > 0 && sequence[0].indexOf('#heart') === -1) {
        // 触发主心粒子分散效果
        if (typeof window.triggerHeartExplosion === 'function') {
          window.triggerHeartExplosion();
        }
      }
      ...
      setTimeout(showImages, 2000);
        ...
        default:
          ...
          // 显示普通文字时触发满屏飘动小心心效果
          if (typeof window.createFloatingHearts === 'function' && current[0] !== cmd) {
            window.createFloatingHearts();
          }
      }
...
S.Dot = function (x, y) {
  ...
  // 随机粉色系颜色 - 七夕主题
  const hue = Math.floor(Math.random()*30) + 330; // 粉色到红色范围
  const lightness = Math.floor(Math.random()*20) + 60; // 亮度
  this.c = new S.Color(
    Math.floor(255),
    Math.floor(255 * (lightness / 100)),
    Math.floor(255 * (lightness / 100) * 0.8),
    this.p.a
  );
  ...
  return {
    ...
    heart: function (maxShapeSize) {
      var dots = [],
          ...
          centerY = shapeCanvas.height/2; // 底部75%位置
